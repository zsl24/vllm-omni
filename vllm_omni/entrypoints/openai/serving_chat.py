...
from vllm_omni.utils.audio import apply_hann_fade_to_chunk, audio_chunk_pcm_bytes, audio_chunk_sample_rate
...
class OmniOpenAIServingChat(OpenAIServingChat, AudioMixin):
    ...
    def _create_audio_choice(
        ...
        # Apply Hann-window fade for streaming audio to eliminate
        # boundary pops/clicks between consecutive chunks.
        is_last_chunk = any(
            o.finish_reason is not None for o in final_res.outputs
        )
        if stream and req_state is not None:
            audio_tensor = apply_hann_fade_to_chunk(
                chunk=audio_tensor,
